  ...
  async *chat(
    ...
    try {
      ...
      if (isSlashCommand) {
        ...
        console.error(`[CraftAgent] Creating query for slash command...`);
        this.currentQuery = query({ prompt: trimmedMessage, options: optionsWithAbort });
        console.error(`[CraftAgent] Query created for slash command, starting iteration...`);
      } else if (hasBinaryAttachments) {
        const sdkMessage = this.buildSDKUserMessage(userMessage, attachments);
        async function* singleMessage(): AsyncIterable<SDKUserMessage> {
          yield sdkMessage;
        }
        console.error(`[CraftAgent] Creating query for binary attachments...`);
        this.currentQuery = query({ prompt: singleMessage(), options: optionsWithAbort });
        console.error(`[CraftAgent] Query created for binary attachments, starting iteration...`);
      } else {
        ...
        console.error(`[CraftAgent] Creating query for text prompt...`);
        this.currentQuery = query({ prompt, options: optionsWithAbort });
        console.error(`[CraftAgent] Query created, starting iteration...`);
      }
      ...
      let messageCount = 0;
      try {
        console.error(`[CraftAgent] Starting iteration over currentQuery...`);
        for await (const message of this.currentQuery) {
          messageCount++;
          console.error(`[CraftAgent] Received message #${messageCount}: type=${('type' in message) ? message.type : 'unknown'}`);
